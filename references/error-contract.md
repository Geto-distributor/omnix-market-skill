...
| 412 | ETag 已变化 | 读取最新 owner draft，做三方合并 |
| 422 | DTO/证据/业务验证失败 | 保留错误，回到 ResearchDelta 修正 |
| 428 | 缺 If-Match | 读取 ETag 后再请求 |
| 429 | 速率限制 | `rate_limited`，遵守 Retry-After |
| 5xx | 服务暂不可用 | `upstream_unavailable`，保留 checkpoint |

重试只适用于幂等读请求、带稳定 Idempotency-Key 的 create，或在重新读取 ETag 后由用户确认的 update。不得自动重试 DELETE、submit 或任何审批操作。
