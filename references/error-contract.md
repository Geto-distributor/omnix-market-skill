...
重试只适用于幂等读请求和带稳定 Idempotency-Key 的 create。update 超时后先 resolve 并回读稳定 draft/resource key，再判断服务端状态；不得自动重试 DELETE、submit 或任何审批操作。Agent 公共合同不使用 ETag/If-Match/412/428。
