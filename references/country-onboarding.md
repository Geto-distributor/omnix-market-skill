...
7. 读取 owner draft overlay，校验 resourceKey、draftKey、ETag、warnings 和关系闭包。
8. 默认停在私人草稿。只有用户明确要求“提交审核”时才调用 `drafts:submit`。Approve/Reject 和发布由 Web UI 完成。

不写 ResearchRun。研究范围、检查点和 ResearchDelta 由上层 GETO Skills 维护，OmniX Market 只接收稳定业务实体与证据。

## 完成判定

一次新国家拓客的 API 写入面具备：主体去重、公司与项目、参与方/产品/关系、证据链、联系人/海关/财务、当前六维评分、私人草稿回读和显式提交。初始化、人工审核和发布是人为治理步骤，不属于 API Key 自动执行范围。
